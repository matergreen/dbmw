...
| **完整 ORM**（关系映射 / 懒加载 / 脏跟踪 / 自动生成 SQL） | 属应用层，中间件应止步于 SQL 与结果集 |
...
> **v0.5.0 修订（2026-09-07）**：第一行的措辞由「ORM / 对象映射」精确化为「**完整 ORM**」，
> 并显式列出其构成要件。v0.5.0 引入的 `include/dbmw/mapping.h` 属于**结果集→实体的显式适配**，
> 不在排除范围内，边界如下（详见 `docs/mapping-design-v0.5.0.md` §0）：
>
> | 适配层做的事（v0.5.0 提供） | ORM 做的事（仍排除） |
> |---|---|
> | 按**业务手写的字段声明**搬运列值 ↔ 成员 | 自动推导表结构 / 关系 |
> | SQL 仍由业务书写 | 自动生成 SQL |
> | 只读映射，无状态、无身份 | 实体身份 / 脏跟踪 / 延迟加载 |
> | 头文件模板，`dbmw.h` 零改动 | 需要引擎内建与生命周期管理 |
>
> 判断标准只有一条：**中间件是否开始替业务决定 SQL 与对象生命周期**。只要答案是否，就仍在适配层。

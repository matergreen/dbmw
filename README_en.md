...
> and validated by 125 behavioral tests in `tests/dbmw_core_test.cpp` (mock driver, no real database needed).
>
...
> **Note**: `DBMW_ENABLE_ODBC` is **ON by default** in this project's `CMakeLists.txt`. On macOS, if unixODBC is not installed, a bare `cmake ..` triggers a `FATAL_ERROR` — either `brew install unixodbc` first, or pass `-DDBMW_ENABLE_ODBC=OFF`.
